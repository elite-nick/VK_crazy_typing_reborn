...
messages = []
typing_type = "typing"
typing_type_text = "Набор текста"
gamlet = 0
gamlet_status = "Выключен"
print(len(peers))
...
while True:
  ...
  if len(peers) == 0:
    ...
    if select == "1":
      ...
      os.system('cls')
  if len(peers) > 0:
    print(color.GREEN + "Статус спама: Работает!\n"
          + color.END +
          "\nТип спама: " + typing_type_text +
          "\nРежим призрака: " + gamlet_status +
    ...
    if select == "1":
      ...
      print("\nВыберите тип спама: \n\n"
         "1. печатает\n2. записывает голосовое сообщение\n3. записывает видеосообщение (почему-то не работает)\n4. отправляет файл\n5. отправляет фото\n6. отправляет видео" 
           )
      select_spam = input()
      if select_spam == "2":
        typing_type = "audiomessage"
        typing_type_text = "Запись аудиосообщения"
        print('Изменено на аудиосообщение')
        time.sleep(1.5)
        os.system('cls')
      elif select_spam == "3":
        typing_type = "videomessage"
        typing_type_text = "Запись видеосообщения"
        print('Изменено на видеосообщение')
        time.sleep(1.5)
        os.system('cls')
      elif select_spam == "4":
        typing_type = "file"
        typing_type_text = "Отправка файла"
        print('Изменено на отправку файла')
        time.sleep(1.5)
        os.system('cls')
      elif select_spam == "5":
        typing_type = "photo"
        typing_type_text = "Отправка фото"
        print('Изменено на отправку фото')
        time.sleep(1.5)
        os.system('cls')
      elif select_spam == "6":
        typing_type = "video"
        typing_type_text = "Отправка видео"
        print('Изменено на отправку видео')
        time.sleep(1.5)
        os.system('cls')        
      else:
        typing_type = "typing"
